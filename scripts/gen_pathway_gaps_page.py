...
""")
    ...
    L.append("Sorted by absolute gap. A large positive Δ usually points to "
             "incomplete indoor OSM connectivity (Valhalla detours); a large "
             "negative Δ suggests an over-cautious GTFS `traversal_time`.\n")
    L.append("| # | From → To | Length | GTFS | Valhalla | Δ |")
    L.append("|---|-----------|-------:|-----:|---------:|--:|")
    for i, r in enumerate(indoor[: args.top], 1):
        ln = f"{r['length_m']:.0f} m" if r["length_m"] else "—"
        L.append(f"| {i} | {name_of(stops, r['from'])} → {name_of(stops, r['to'])} | "
                 f"{ln} | {r['gtfs_traversal_s']} s | {r['valhalla_s']} s | "
                 f"**{r['diff_s']:+d} s** |")
    L.append("")
